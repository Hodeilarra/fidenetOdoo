odoo.define("repair_order_quotation.CreateQuotationButton", function(require) {
    "use strict";

    const RepairComponent = require('repair.RepairComponent');
    const ProductScreen = require('repair.ProductScreen');
    const {
        useListener
    } = require('web.custom_hooks');
    const Registries = require('repair.Registries');

    class CreateQuotationButton extends RepairComponent {
        ...
        get client() {
            return this.env.repair.get_client();
        }

        get currentOrder() {
            return this.env.repair.get_order();
        }
            ...
            if (confirmed) {
                ...
                try {
                    this.rpc({
                        ...
                        if (result) {
                            self.env.repair.quotation_number = result[1];
                            ...
                            self.env.repair.db.add_quotations(result[0]);
                        }
                    });
                } catch (error) {
                    this.showPopup('QuotationPopUpAlert', {
        ...
        formatCurrentOrder(payload, quotation_number) {
            const cashier = this.env.repair.get_cashier()
            ...
            let val = {
                ...
                repair_session_id: order.repair_session_id,
