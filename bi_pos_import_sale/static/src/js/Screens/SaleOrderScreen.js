odoo.define('bi_pos_import_sale.SaleOrderScreen', function (require) {
	'use strict';
	...
	class SaleOrderScreen extends PosComponent {
		constructor() {
			...
			this.updateOrderList = debounce(this.updateOrderList, 5);
		}
		...
		search_orders(saleorders,query){
			...
			if(search_text){
				saleorders.forEach(function(odr) {
					if(odr.codigo_barras==false){
						odr.codigo_barras="";
					}
					if (((odr.name.toLowerCase()).indexOf(search_text) != -1) || 
						((odr.codigo_barras.toLowerCase()).indexOf(search_text.toLowerCase()) != -1) || 
