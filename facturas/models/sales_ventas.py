...
class sales_ventas(models.Model):
    ...
    related_purchase_count        = fields.Integer(compute='_compute_has_related_purchase', string='Compras relacionadas', store=True)
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        res = super(sales_ventas, self).search_read(domain, fields, offset, limit, order)
        if limit :  
            suma_base_imponible = sum(self.search(domain).mapped('amount_untaxed'))
            suma_impuestos = sum(self.search(domain).mapped('amount_tax'))
