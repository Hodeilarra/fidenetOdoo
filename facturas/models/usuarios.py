...
class ResPartner(models.Model):
    ...
    custom_sales_ventas_count                 = fields.Integer(string='Presupuestos', compute='_compute_custom_sales_ventas_count')
        
    custom_sales_compras_ids                  = fields.One2many('facturas.sales_compras', 'customer', string='Presupuestos de compra')
    custom_sales_compras_count                = fields.Integer(string='Presupuestos', compute='_compute_custom_sales_compras_count')


    custom_facturas_cliente_ids               = fields.One2many('facturas.invoice', 'customer', string='Facturas cliente')
    custom_facturas_cliente_total             = fields.Float(string='Facturas cliente', compute='_compute_custom_facturas_cliente_count')

    custom_facturas_proveedor_ids             = fields.One2many('facturas.invoice', 'supplier_id', string='Facturas proveedor')
    custom_facturas_proveedor_total           = fields.Float(string='Facturas proveedor', compute='_compute_custom_facturas_proveedor_count')
