...
#     _name = 'facturas.facturas'
#     _description = 'facturas.facturas'
...
class invoice(models.Model):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        res = super(invoice, self).search_read(domain, fields, offset, limit, order)
        if limit:
            # Agregar la función read_group para calcular los totales
            #totals = self.read_group(domain, ['amount_untaxed', 'amount_total', 'total_comision'], [])
            suma_base_imponible = sum(self.search(domain).mapped('amount_untaxed'))
            suma_total = sum(self.search(domain).mapped('amount_total'))
            suma_comision = sum(self.search(domain).mapped('total_comision'))
            try:
                # Agregar un nuevo diccionario a la lista de resultados con los valores calculados
                res.append({
                    'sequence': 'TOTAL:',
                    'customer': '',
                    'supplier_id': '',
                    ...
                    'invoice_date_due': '',
                    'comision': '',                    
                    'amount_untaxed': suma_base_imponible,
                    'amount_total': suma_total,
