...
odoo.define('pos_coupon.pos_coupon', function(require) {
    ...
    var CreateconfirmPopupWidget = PopupWidget.extend({
        template: 'CreateconfirmPopupWidget',
            ...
            this.$('#print-coupons').off('click').click(function() {
                if (self.pos.config.iface_print_via_proxy) {
                        ...
                        .then(function(result){
                            ...
                            self.pos.proxy.printer.print_receipt(t);
                        });
    ...
    gui.define_popup({
        name: 'create-confirm-screen',
        widget: CreateconfirmPopupWidget
    });
    ...
    var CreateCouponPopupWidget = PopupWidget.extend({
        ...
        saveBackend: function(name, validity, availability, coupon_value, note, customer_type, partner_id, voucher_usage, amount_type, max_expiry_date, redeemption_limit, partial_redeem) {
            ...
            .then(function(result){
                self.gui.show_popup('create-confirm-screen', {
                        'wk_id': result,
                });
            });
        },
                    ...
                    if (wk_order != null) {
                        ...
                        for (var i = 0; i < wk_order.orderlines.models.length; i++) {
                            ...
                            if (wk_order.orderlines.models[i].product.id == wk_product_id) {
                                ...
                                if(server_ids.order_ids){
                                    var ids = server_ids.order_ids
                                }
                                else{
                                    var ids = server_ids
                                }
                                ...
                                rpc.query({
                                    ...
                                    args :  [coupon_id, wk_voucher_value, ids[0], wk_order.orderlines.models[i].id, client_id],
                                    }).catch(function(unused, event) {
                                        self.gui.show_popup('error', {
                                            'title': _t('Error !!!'),
                                            'body': _t("Connection Error. Try again later !!!"),
