...
odoo.define("pos_debt_notebook.pos", function (require) {
                ...
                async function () {
                    ...
                    if (client) {
                        ...
                        for(let i=0; i<this.orderlines["models"].length; i++){
                            console.log("Valor: "+this.orderlines.models[i].price*this.orderlines.models[i].quantity);
                            let valorLinea = this.orderlines.models[i].price*this.orderlines.models[i].quantity
                            precioCompra = precioCompra + valorLinea;
                        }
                        if(Math.abs(client.debt)<precioCompra){
                           alert("El total de la compra excede el crédito del cliente.")
                        }
                        
                        console.log("El credito del cliente es: "+client.debt);
    ...
    //AÑADIMOS CONTROL SOBRE EL CRÉDITO
    ...
    class extends ProductScreen {
        ...
        async _clickProduct(event) {
            const result = super._clickProduct(event);
            setTimeout(() => this._validateCredit(), 0);
            return result;
        }

        async _updateOrderline(event) {
            const result = super._updateOrderline(event);
            setTimeout(() => this._validateCredit(), 0);
            return result;
        }

        async _deleteOrderline(event) {
            const result = super._deleteOrderline(event);
            setTimeout(() => this._validateCredit(), 0);
            return result;
        }

        async _validateCredit() {
            ...
            if (order && client) {
                ...
                let debt_company = await rpc.query({
                    model: 'res.partner',
                    method: 'read',
                    args: [client.id, ['debt_company']],
                });

                if (debt_company) {
                    
                    if (total > Math.abs(debt_company[0].debt_company) && !this.creditWarningShown) {
                        this.showPopup('ErrorPopup', {
                            title: 'Advertencia de crédito',
                            body: 'El total de la compra excede el crédito del cliente.',
                        });
                        this.creditWarningShown = true;
                    } else if (total <= Math.abs(debt_company[0].debt_company)) {
                        this.creditWarningShown = false;
                    }
                }
            }
